...
3. **徽章组合规则**(卡片 / 桌面 TOC / 移动 TOC / 气泡四处共用,`badgesFor`):
   - 无段位 → 状态单徽章;
   - 有段位 且 状态 ∈ {已通关, 全成就, 暂退长草, 已退役} → **双徽章**(状态 + 段位);
   - 有段位 且 状态 ∈ {持续游玩, 未通关} → 段位单徽章。
...
- 双徽章规则用 `my_rank` 隐式表达"联机有战绩",不加 `is_online_active` 布尔字段;单徽章规则避免竞技游戏出现 [持续游玩] + [段位] 两个近似徽章。
- 暂退长草/已退役 + 段位 → 双徽章:状态本身有信息量(已不活跃),段位是历史记录,并排不误导。
