...
流程:输入游戏名关键词或 appid 选择游戏 → 六阶梯游玩状态菜单(回车保持当前值)→ 历史最高段位(回车跳过)→ 短评(回车跳过)→ 长评链接(回车跳过)→ 游玩年份(回车跳过)→ 平台(回车默认 PC)。跳过的字段不写入、不覆盖;自动字段(中文名/tags/发售日期)永不触碰。写入结果与手动编辑 JSON 完全一致。
...
- **段位双徽章规则**(ADR-0007):填了 `my_rank` 的游戏——已通关/全成就/暂退长草/已退役 → 状态徽章 + 段位徽章(翡翠绿带边框)并排;持续游玩/未通关 → 只显示段位徽章。无段位 → 只显示状态徽章。卡片、右侧 TOC、移动 TOC、气泡四处一致。
...
| 填了段位但卡片只显示段位,看不到"持续游玩" | 双徽章规则:持续游玩/未通关 + 段位 → 只显示段位 | 属正常;想同时显示状态徽章,把状态改为 已通关/全成就 |
