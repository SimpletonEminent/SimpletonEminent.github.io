...
> **💡 统一推荐入口**：在项目根目录运行 **`npm run updatedata`** 即可进入交互式控制台，输入对应数字即可调起所有手动作业（含环境自检与引导）。原有独立命令仍保持可用。
...
### 4. 手写注释(游玩状态/段位/短评/tags 特色词/长评链接)

**推荐方式:表格化批量编辑(数百款游戏时)**
1. 项目根目录运行 `npm run json-to-csv` → 生成 `steam.csv`(UTF-8,Excel/WPS 打开中文不乱码)
2. 用 Excel/WPS 打开,表格含列:appid(只读)/ 游戏名(只读)/ 游玩状态 / 最高段位 / 短评 / 长评链接 / 游玩年份 / 平台 / 特色标签
3. 批量填充:游玩状态列直接下拉选(未通关/已通关/全成就/持续游玩/暂退长草/已退役),最高段位填自由文本(如 `璀璨钻石 💎`,可空),平台列下拉选(PC/PlayStation/Xbox/Switch),特色标签用分号分隔(如 `魂系;开放世界`)
...
```json
{
  "570": {
    ...
    "my_status": "ongoing",                 // 手写六阶梯:uncompleted / completed / perfect / ongoing(持续游玩)/ hiatus(暂退长草)/ retired(已退役)
    "my_rank": "冠绝一世",                  // 手写,竞技/网游历史最高段位(可空;空则只显示状态徽章)
    "my_review": "你的短评",                // 手写,空则不显示
    "blog_url": "/blog/dota2-review",       // 手写,长评路由(小写)
    "play_year": "2013",                    // 手写,游玩年份(单年如 2013,或区间如 "2021-2026")
    "platform": "PC",                       // 手写
...
**关键原则**:每日同步脚本**永不触碰**此文件。你的手写数据(游玩状态/段位/短评/tags 特色词/长评链接/游玩年份)丢不得,自动脚本也不会动它们。

### 4.5 交互式 CLI 录入(单款快速录入)

不想开 Excel 时,用命令行交互录入:

```powershell
npm run review
```

流程:输入游戏名关键词或 appid 选择游戏 → 六阶梯游玩状态菜单(回车保持当前值)→ 历史最高段位(回车跳过)→ 短评(回车跳过)→ 长评链接(回车跳过)→ 游玩年份(回车跳过;支持单年如 2024 或区间如 2021-2026,非法值自动跳过不写入)→ 平台(回车默认 PC)。跳过的字段不写入、不覆盖;自动字段(中文名/tags/发售日期)永不触碰。写入结果与手动编辑 JSON 完全一致。
...
| 填了段位后"持续游玩"徽章不见了 | 旧规则(ADR-0007 初版)会让段位顶替状态;已改为状态徽章永远显示 + 段位追加 | 升级到 ADR-0007 v2 后自动恢复 |
